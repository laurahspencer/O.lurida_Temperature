...
#treats.main <- c("Wild", "Cold / Low Food", "Warm / Low Food", "Cold / High Food", "Warm / High Food")
...
for (i in 1:length(treats)) {
  malestage.treats[[i]] <- as.data.frame(prop.table(table(subset(histo, c(TREAT==treats[i] | TREAT=="PRE"))$Week, subset(histo, c(TREAT==treats[i] | TREAT=="PRE"))$MALSTAGE.COL2), margin=1))
